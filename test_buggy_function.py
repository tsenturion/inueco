...
def test_failed_students_calculation():
    ...
    # ОШИБКА: в функции условие <= 60 вместо < 60
    # Поэтому Студент2 тоже попадет в неуспевающие
    assert "Студент2" in result["failed_students"]  # Это ошибка!
    assert len(result["failed_students"]) == 3  # Все три студента считаются неуспевающими
...
def test_adults_count_calculation():
    ...
    # ОШИБКА: в функции условие > 18 вместо >= 18
    # Поэтому студент 18 лет не считается совершеннолетним
    assert result["adults_count"] == 1  # Только один студент (19 лет)
...
def test_top_student_determination():
    ...
    assert result["average_score"] == 73.33
...
def test_edge_case_60_score():
    ...
    # Студент со средним 60 НЕ должен быть неуспевающим
    # Но из-за ошибки в условии (<= 60) он попадет в неуспевающие
    assert result["failed_students"] == ["Пограничный"]  # Это ошибка!
...
def test_edge_case_18_years():
    ...
    # Студент 18 лет ДОЛЖЕН считаться совершеннолетним
    # Но из-за ошибки в условии (> 18) он не считается
    assert result["adults_count"] == 0  # Это ошибка!