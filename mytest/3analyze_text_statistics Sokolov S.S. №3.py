...
class TestBook:
    ...
    def test_is_available_should_return_true_when_available_copies_exist(self, sample_book):
        assert sample_book.is_available() is True

    def test_is_available_should_return_false_when_no_available_copies(self):
...
class TestReader:
    ...
    def test_history_should_record_operations_with_timestamps(self, sample_reader):
        """Проверяет, что история корректно записывает операции с временными метками"""
...
class TestLibrary:
    ...
    def test_borrow_book_should_return_false_when_book_not_available(self, library_with_data):
    ...
    def test_active_loans_should_contain_correct_return_date(self, library_with_data):
        library_with_data.borrow_book("R001", "978-0134685991")
        due_date = library_with_data.active_loans["R001"]["978-0134685991"]
        expected_due_date = datetime.now() + timedelta(days=Library.LOAN_PERIOD_DAYS)
        # Проверяем, что даты совпадают с точностью до секунды
        assert abs((due_date - expected_due_date).total_seconds()) < 2
    ...
    def test_return_book_should_successfully_return_without_fine(self, library_with_data):
        ...
        assert "R001" not in library_with_data.active_loans or "978-0134685991" not in library_with_data.active_loans.get("R001", {})
